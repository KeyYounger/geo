...
if model_path == '': # if no pre-trained model path
    prefix = '%s_%s'%('Model_TrGNN', int(start_time))
    checkpoint_epoch = -1
...
# if dataset == 'demo':
start_date, end_date = '2008-05-17', '2008-05-17'
calibrate = False
...
flow_df = pd.concat([pd.read_csv('data/flow.csv', index_col=0) for date in dates])
