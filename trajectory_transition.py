...
def extract_trajectory_transition(start_date, end_date, interval=15):
    ...
    total_file_path = 'data/trajectory_transition.pkl'
        ...
        for current_date in date_list:
            ...
            if os.path.exists(file_path):
                ...
                def extract_time_index(time):
                    ...
                    return time_index
                recovered_trajectory_df['time_index'] = recovered_trajectory_df['timestamp'].apply(extract_time_index)
                print('Time spent till now: %.2f seconds'%(time.time() - start_time))
                ...
                for i, row in recovered_trajectory_df.iterrows():
                    if i != 0:
                        if previous_row['taxi_id'] == row['taxi_id'] and \
                        ...
                        previous_row['road_id'] != row['road_id']:
                            ...
                            print(trajectory_transition[previous_row['time_index'], previous_row['road_index'], row['road_index']])
                    if i % 100000 == 0:
